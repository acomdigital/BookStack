...
function clike(parserConfig) {
  ...
  function tokenBase12(stream, state) {
    ...
    if (contains(keywords13, cur)) {
      if (contains(blockKeywords, cur)) curPunc3 = "newstatement";
    ...
    if (contains(atoms5, cur)) return "atom";
    return "variable";
  }
  ...
  return {
    ...
    token: function(stream, state) {
      ...
      if (curPunc3 == ";" || curPunc3 == ":" || curPunc3 == "," && stream.match(/^\s*(?:\/\/.*)?$/, false))
      ...
      else if (curPunc3 == "{") pushContext(state, stream.column(), "}");
      else if (curPunc3 == "[") pushContext(state, stream.column(), "]");
      else if (curPunc3 == "(") pushContext(state, stream.column(), ")");
      else if (curPunc3 == "}") {
        ...
        while (ctx.type == "statement") ctx = popContext(state);
      } else if (curPunc3 == ctx.type) popContext(state);
...
function words(str) {
  ...
  for (var i = 0; i < words7.length; ++i) obj[words7[i]] = true;
  return obj;
}
      ...
      return function(stream, state) {
        ...
        if (inner == "punctuation") {
          if (stream.current() == "{") {
            ...
            else state.tokenize = tokenString7;
          }
        }
        return inner;
      };
...
function wordObj(words7) {
  var o = {};
  for (var i = 0, e = words7.length; i < e; ++i) o[words7[i]] = true;
  return o;
}
var keywordList = [
  "alias",
...
var ruby = {
  ...
  token: function(stream, state) {
    ...
    if (curPunc2 || style && style != "comment") state.lastTok = thisTok;
    if (curPunc2 == "|") state.varList = !state.varList;
  ...
  indent: function(state, textAfter, cx) {
    if (state.tokenize[state.tokenize.length - 1] != tokenBase8) return null;
    var firstChar = textAfter && textAfter.charAt(0);
...
function sql(parserConfig) {
  ...
  function tokenBase12(stream, state) {
    ...
    if (support.hexNumber && (ch == "0" && stream.match(/^[xX][0-9a-fA-F]+/) || (ch == "x" || ch == "X") && stream.match(/^'[0-9a-fA-F]*'/))) {
      ...
      if (dateSQL.hasOwnProperty(word) && (stream.match(/^( )+'[^']*'/) || stream.match(/^( )+"[^"]*"/)))
        return "number";
      if (atoms5.hasOwnProperty(word)) return "atom";
      if (builtin.hasOwnProperty(word)) return "type";
      if (keywords13.hasOwnProperty(word)) return "keyword";
  ...
  function tokenComment7(depth) {
    return function(stream, state) {
      ...
      else state.tokenize = tokenBase12;
      return "comment";
    };
  }
  ...
  return {
    ...
    token: function(stream, state) {
      ...
      if (state.tokenize == tokenBase12 && stream.eatSpace()) return null;
      var style = state.tokenize(stream, state);
      if (style == "comment") return style;
      if (state.context && state.context.align == null)
...
function wordSet(words7) {
  var set2 = {};
  for (var i = 0; i < words7.length; i++) set2[words7[i]] = true;
  return set2;
}
...
function tokenBase10(stream, state, prev) {
  ...
  if (stream.match(identifier)) {
    ...
    if (atoms4.hasOwnProperty(ident)) return "atom";
